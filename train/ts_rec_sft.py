...
from data.ts_rec_data import SidSFTDataset, SidItemFeatDataset, FusionSeqRecDataset, SidTokenFeatDataset
import random
...
def train(
    ...
    use_sa_init: bool = True,
    use_ts_align: bool = True
):
    set_seed(seed)
    os.environ['WANDB_PROJECT'] = wandb_project

    category_dict = {"Industrial_and_Scientific": "industrial and scientific", "Office_Products": "office products", "Toys_and_Games": "toys and games"}
    ...
    # -------------------------------- SA-Init --------------------------------
    if use_sa_init:
        assert description_path and os.path.exists(description_path), \
            f"SA-Init requires --description_path, got: {description_path}"
        ...
        print(f"SA-Init: initialized {len(prefix_description_text)} SID tokens with semantic embeddings.")
    else:
        print("SA-Init: disabled, using default random initialization for SID tokens.")
    # -------------------------------- SA-Init --------------------------------
    ...
    if use_ts_align:
        assert description_path and os.path.exists(description_path), \
            f"TS-Align requires --description_path, got: {description_path}"
        ...
        print(f"TS-Align: enabled, added {len(train_data4)} token-semantic alignment samples.")
    else:
        print("TS-Align: disabled.")
    # -------------------------------- TS-Align --------------------------------
